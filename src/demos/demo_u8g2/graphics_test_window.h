...
#ifndef _GRAPHICS_TEST_WINDOW_H
#define _GRAPHICS_TEST_WINDOW_H
...
class GraphicsTestWindow : public U8g2Window {
public:
    ...
    void Start() {
        xTaskCreate([](void *pvParam) {
            GraphicsTestWindow* this_ = (GraphicsTestWindow*)pvParam;
            while(1) {
                // picture loop  
                this_->u8g2->clearBuffer();
                this_->draw();
                this_->u8g2->sendBuffer();
                
                // increase the state
                this_->draw_state++;
                if ( this_->draw_state >= 12*8 )
                    this_->draw_state = 0;

                // delay between each page
                delay(100);
            }
        }, "Test_Task", 16384, this, 1, NULL);
    }
