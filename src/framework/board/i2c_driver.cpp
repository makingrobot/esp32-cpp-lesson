...
I2cDriver::I2cDriver(i2c_master_bus_handle_t i2c_bus, uint8_t addr) {
...
void I2cDriver::WriteReg(uint8_t reg, uint8_t value) {
    uint8_t buffer[2] = {reg, value};
    ESP_ERROR_CHECK(i2c_master_transmit(i2c_device_, buffer, 2, 100));
}

uint8_t I2cDriver::ReadReg(uint8_t reg) {
    uint8_t buffer[1];
    ESP_ERROR_CHECK(i2c_master_transmit_receive(i2c_device_, &reg, 1, buffer, 1, 100));
    return buffer[0];
}

void I2cDriver::ReadRegs(uint8_t reg, uint8_t* buffer, size_t length) {
