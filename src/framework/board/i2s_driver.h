...
public:
    ...
    i2s_chan_handle_t txChan() const { return tx_chan; }
    uint32_t txSampleRate() const { return tx_sample_rate; }
    i2s_data_bit_width_t txDataWidth() const { return tx_data_bit_width; }
    i2s_slot_mode_t txSlotMode() const { return tx_slot_mode; }

    i2s_chan_handle_t rxChan() const { return rx_chan; }
    uint32_t rxSampleRate() const { return rx_sample_rate; }
    i2s_data_bit_width_t rxDataWidth() const { return rx_data_bit_width; }
    i2s_slot_mode_t rxSlotMode() const { return rx_slot_mode; }

    int lastError() const { return (int)last_error; }
