...
bool I2sDriver::i2sDetachBus(void *bus_pointer)
{
    I2sDriver *bus = (I2sDriver *)bus_pointer;
...
void I2sDriver::setPins(int8_t bclk, int8_t ws, int8_t dout, int8_t din, int8_t mclk)
{
...
void I2sDriver::setInverted(bool bclk, bool ws, bool mclk)
{
    _mclk_inv = mclk;
    _bclk_inv = bclk;
    _ws_inv = ws;
...
#if SOC_I2S_SUPPORTS_PDM_TX
void I2sDriver::setPinsPdmTx(int8_t clk, int8_t dout0, int8_t dout1)
{
...
#if SOC_I2S_SUPPORTS_PDM_RX
void I2sDriver::setPinsPdmRx(int8_t clk, int8_t din0, int8_t din1, int8_t din2, int8_t din3)
{
...
void I2sDriver::setInvertedPdm(bool clk)
{
#if SOC_I2S_SUPPORTS_PDM_TX
    _tx_clk_inv = clk;
#endif
...
bool I2sDriver::initSTD(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch, int8_t slot_mask)
{
    ...
    if (_mclk >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_STD_MCLK, I2sDriver::i2sDetachBus);
    }
    if (_bclk >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_STD_BCLK, I2sDriver::i2sDetachBus);
    }
    if (_ws >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_STD_WS, I2sDriver::i2sDetachBus);
    }
    if (_dout >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_STD_DOUT, I2sDriver::i2sDetachBus);
    }
    if (_din >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_STD_DIN, I2sDriver::i2sDetachBus);
    }

    // I2S configuration
    if (_dout >= 0 && _din >= 0)
    {
        i2s_chan_config_t chan_cfg = I2S_CHAN_CFG(I2S_NUM_AUTO);
        I2S_ERROR_CHECK_RETURN_FALSE(i2s_new_channel(&chan_cfg, &tx_chan, &rx_chan));
    }
    else if (_dout >= 0)
    {
        i2s_chan_config_t chan_cfg = I2S_CHAN_CFG(I2S_NUM_1);
        I2S_ERROR_CHECK_RETURN_FALSE(i2s_new_channel(&chan_cfg, &tx_chan, NULL));
    }
    else if (_din >= 0)
    {
        i2s_chan_config_t chan_cfg = I2S_CHAN_CFG(I2S_NUM_0);
...
err:
    ...
    I2sDriver::i2sDetachBus((void *)(this));
    return false;
}

#if SOC_I2S_SUPPORTS_TDM
bool I2sDriver::initTDM(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch, int8_t slot_mask)
{
    // Peripheral manager deinit previous peripheral if pin was used
    if (_mclk >= 0)
    {
    ...
    if (_mclk >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_TDM_MCLK, I2sDriver::i2sDetachBus);
    }
    if (_bclk >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_TDM_BCLK, I2sDriver::i2sDetachBus);
    }
    if (_ws >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_TDM_WS, I2sDriver::i2sDetachBus);
    }
    if (_dout >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_TDM_DOUT, I2sDriver::i2sDetachBus);
    }
    if (_din >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_TDM_DIN, I2sDriver::i2sDetachBus);
    }
...
#if SOC_I2S_SUPPORTS_PDM_TX
bool I2sDriver::initPDMtx(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch)
{
    // Peripheral manager deinit previous peripheral if pin was used
    if (_tx_clk >= 0)
    {
    ...
    if (_tx_clk >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_TX_CLK, I2sDriver::i2sDetachBus);
    }
    if (_tx_dout0 >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_TX_DOUT0, I2sDriver::i2sDetachBus);
    }
    if (_tx_dout1 >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_TX_DOUT1, I2sDriver::i2sDetachBus);
    }
...
#if SOC_I2S_SUPPORTS_PDM_RX
bool I2sDriver::initPDMrx(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch)
{
    // Peripheral manager deinit previous peripheral if pin was used
    if (_rx_clk >= 0)
    {
    ...
    if (_rx_clk >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_RX_CLK, I2sDriver::i2sDetachBus);
    }
    if (_rx_din0 >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_RX_DIN0, I2sDriver::i2sDetachBus);
    }
    if (_rx_din1 >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_RX_DIN1, I2sDriver::i2sDetachBus);
    }
    if (_rx_din2 >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_RX_DIN2, I2sDriver::i2sDetachBus);
    }
    if (_rx_din3 >= 0)
    {
        perimanSetBusDeinit(ESP32_BUS_TYPE_I2S_PDM_RX_DIN3, I2sDriver::i2sDetachBus);
    }
...
bool I2sDriver::begin(i2s_mode_t mode, uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch, int8_t slot_mask)
{
...
bool I2sDriver::configureTX(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch, int8_t slot_mask)
{
    /* Setup I2S channels */
    if (tx_chan != NULL)
    {
...
bool I2sDriver::configureRX(uint32_t rate, i2s_data_bit_width_t bits_cfg, i2s_slot_mode_t ch, i2s_rx_transform_t transform)
{
    /* Setup I2S channels */
    if (rx_chan != NULL)
    {
...
size_t I2sDriver::readBytes(char *buffer, size_t size)
{
    size_t readed = 0;
    size_t bytes_read = 0;
    ...
    while (readed < size)
    {
        ...
        I2S_ERROR_CHECK_RETURN(rx_fn(rx_chan, rx_transform_buf, (char *)(buffer + readed), bytes_to_read, &bytes_read, _timeout), 0);
        readed += bytes_read;
    }
    return readed;
}

size_t I2sDriver::write(const uint8_t *buffer, size_t size)
{
    size_t written = 0;
    size_t bytes_sent = 0;
...
bool I2sDriver::transformRX(i2s_rx_transform_t transform)
{
    switch (transform)
    {
...
bool I2sDriver::allocTranformRX(size_t buf_len)
{
    char *buf = NULL;
    if (buf_len == 0)
    {
